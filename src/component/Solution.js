...
const Solution = () => {
  const [slideNum, setNum] = useState(0);
        ...
        <div className="container">
              ...
              SOLUTION.map((img, idx) => {
                return (
                  <figure key={img.id} className={slideNum === idx ? 'on' : ''}>
                    <img src={process.env.PUBLIC_URL + `/assets/images/main_s0${img.id}.jpg`} alt="hyundai" />
                  </figure>
                )
              })
            }
          ...
          <div className="box">
            <div className="content">
              <h3 className="tit">{SOLUTION[slideNum].content}</h3>
          ...
          <div className="arrows">
            <i className="xi-arrow-left" onClick={() => (slide_ref.current.slickPrev())}></i>
            <i className="xi-arrow-right" onClick={() => (slide_ref.current.slickNext())}></i>
          </div>
        </div>
        <ul className="dots">
          {
            SOLUTION.map((dot, idx) => <li key={dot.id} onClick={() => slide_ref.current.slickGoTo(idx)} className={slideNum === idx ? "on" : " "}>
              {dot.id}
            </li>)
          }
        </ul>
